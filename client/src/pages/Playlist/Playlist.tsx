    ...
    return (
        <div className='playlist-grid' >
            <div className='playlist-info'>
                ...
                {
                    auth.userId !== loadedPlaylist.creator
                    ?
                        null
                    :
                        <Tippy placement='bottom' arrow={true} delay={0} content='Edit playlist'>
                            <button className="fas fa-ellipsis-h" onClick={handleToggle} style={{ background: 'none', border: 'none', cursor: 'pointer' }} ></button>
                        </Tippy>
                }

                <CModal width={'400px'} title={`Edit playlist - ${loadedPlaylist.name}`} onClose={() => setToggleEdit(p => !p)} show={toggleEdit}>
                ...
                <CModal width={'500px'} title={`Edit playlist - ${loadedPlaylist.name}`} onClose={() => setToggleButton(p => !p)} show={toggleButton}>
                    ...
                    <div>
                        ...
                        <form>
                            <Button type='submit' color=''>
                                Make { loadedPrivacy === true ? 'public' : 'private' }
                            </Button>
                        </form>
                    </div>
                </CModal>
